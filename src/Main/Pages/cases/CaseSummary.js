...
const CardSkeleton = ({ delay = 0 }) => (
  <div
    className="card border-0 shadow-sm h-100 skeleton-animation"
    style={{
      background: "linear-gradient(135deg, #6b7280, #9ca3af)",
      borderRadius: "12px",
...
const CaseSummary = () => {
  ...
  useEffect(() => {
    const fetchDataProgressively = async () => {
      try {
        ...
        // First, show skeleton UI immediately
        setLoadedCharts({
          totalCases: false,
          caseStatus: false,
          financialOverview: false,
        ...
        // 1. First load the most critical data (case summary)
        ...
        // 2. Load closed cases data after a short delay
        setTimeout(async () => {
          try {
            const closedResponse = await axios.get(`${ApiEndPoint}closed-groups`);
            setCasesData(prev => ({
              ...prev,
        ...
        // 3. Load case dates data
        setTimeout(async () => {
          try {
            const datesResponse = await axios.get(`${ApiEndPoint}case-dates`);
            setCaseDatesData(datesResponse.data.casesDates || []);
            markChartLoaded('keyDates');
          } catch (err) {
            console.error("Error loading case dates:", err);
          }
        }, 500);

        // 4. Mark remaining charts as loaded progressively
        setTimeout(() => markChartLoaded('caseStatus'), 700);
        setTimeout(() => markChartLoaded('financialOverview'), 900);
        setTimeout(() => markChartLoaded('casePriority'), 1100);
        setTimeout(() => markChartLoaded('assignedUser'), 1300);

        setLastUpdated(new Date().toLocaleString());

        // 5. Set a timeout to ensure loading state eventually ends
        setTimeout(() => {
          setLoading(false);
        }, 2000);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // Mark remaining charts as loaded with slight delays for better UX
      setTimeout(() => markChartLoaded('caseStatus'), 200);
      setTimeout(() => markChartLoaded('financialOverview'), 400);
      setTimeout(() => markChartLoaded('casePriority'), 600);
      setTimeout(() => markChartLoaded('assignedUser'), 800);

      // End loading after all charts should be loaded
      setTimeout(() => {
        setLoading(false);
      }, 1000);
