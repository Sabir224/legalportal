...
const CaseSummary = () => {
  ...
  const handleExportExcel = () => {
    ...
    let csvContent =
      'Case Number,Status,Priority,Case Type,Request Date,E-Submit Date,First Session Date,Next Session Date,Verdict Date,Claimed Amount,Case Balance\n';
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (filters.startDate || filters.endDate) {
        ...
        const allDates = casesData.cases.map((c) => new Date(c.CreatedAt)).filter((d) => !isNaN(d.getTime()));
    ...
    return (
      <Modal show={show} onHide={onHide} centered size="lg">
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={onHide} style={{ backgroundColor: '#001f3f', borderColor: '#001f3f' }}>
            Apply Filter
          </Button>
        </Modal.Footer>
      </Modal>
  ...
  const filteredCounts = {
    ...
    successRate:
      casesData.backendClosedPositive + casesData.backendClosedNegative > 0
        ? Math.round(
            (casesData.backendClosedPositive / (casesData.backendClosedPositive + casesData.backendClosedNegative)) *
              100
          )
        : 0,
  };
  ...
  const roleOptions = [...new Set(casesData.assignedUsers.map((u) => u.role))].filter(
    (role) => role && role.trim() !== ''
  );

  // Categorize users into Lawyers and Ops
  const lawyers = casesData.assignedUsers.filter((user) => user.role && user.role.toLowerCase().includes('lawyer'));
  const ops = casesData.assignedUsers.filter((user) => user.role && !user.role.toLowerCase().includes('lawyer'));
  ...
  const getAllFilteredKeyDates = (cases) => {
    ...
    if (keyDatesFilter.startDate || keyDatesFilter.endDate) {
      const allDates =
        caseDatesData.length > 0 ? getAllKeyDatesWithoutLimit(caseDatesData) : getAllKeyDatesWithoutLimit(cases);
    ...
    const allDates = caseDatesData.length > 0 ? getAllKeyDates(caseDatesData) : getAllKeyDates(cases);
    ...
    datasets: [
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return (caseDatesData.length > 0 ? caseDatesData : filteredCases).filter(
            (c) => c.RequestDate && new Date(c.RequestDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
      ...
      },
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return caseDatesData.filter(
            (c) => c.ESubmitDate && new Date(c.ESubmitDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
      ...
      },
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return caseDatesData.filter(
            (c) => c.FirstSessionDate && new Date(c.FirstSessionDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
      ...
      },
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return caseDatesData.filter(
            (c) => c.NextSessionDate && new Date(c.NextSessionDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
      ...
      },
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return caseDatesData.filter(
            (c) => c.LastSessionDate && new Date(c.LastSessionDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
      ...
      },
      {
        ...
        data: keyDates.map((date) => {
          const dateStr = date.toISOString().split('T')[0];
          return caseDatesData.filter(
            (c) => c.VerdictDate && new Date(c.VerdictDate).toISOString().split('T')[0] === dateStr
          ).length;
        }),
              ...
              <Form.Group>
                ...
                >
                  ...
                  <optgroup label="Operations">
                    ...
                    {/* Regular Ops */}
                    {ops
                      .filter((user) => !['admin', 'receptionist', 'paralegal'].includes(user.role.toLowerCase()))
                      .map((user) => (
                        <option key={user.userId} value={user.userId}>
                          {user.name} ({user.role})
                        </option>
                  ...
                  <optgroup label="Receptionist">
                    {ops
                      .filter((user) => user.role.toLowerCase() === 'receptionist')
                      .map((user) => (
                        <option key={user.userId} value={user.userId}>
                          {user.name} ({user.role})
                        </option>
                  ...
                  <optgroup label="Paralegal">
                    {ops
                      .filter((user) => user.role.toLowerCase() === 'paralegal')
                      .map((user) => (
                        <option key={user.userId} value={user.userId}>
                          {user.name} ({user.role})
                        </option>
                                      ...
                                      callbacks: {
                                        label: function (context) {
                                          ...
                                          const casesForDate = caseDatesData.filter((c) => {
                                            ...
                                            return (
                                              dateField && new Date(dateField).toISOString().split('T')[0] === dateStr
                                            );
                                          });

                                          const caseNumbers = casesForDate
                                            .map((c) => c.CaseNumber || `Case ${c.FkCaseId || c._id}`)
                                            .join(', ');

                                          return `${label}: ${value} case(s) - ${
                                            caseNumbers || 'No case numbers available'
                                          }`;
                                        },
                                      },
                                    },
                        ...
                        <div className="card-header bg-transparent border-0 p-3">
                          <div className="d-flex justify-content-between align-items-center">
                            <h5 className="mb-0 text-white">
                              ...
                              {filters.assignedUser === 'all-lawyers'
                                ? 'All Lawyers Cases'
                                : filters.assignedUser === 'all-ops'
                                ? 'All Ops Cases'
                                      ...
                                      tooltip: {
                                        callbacks: {
                                          label: function (context) {
                                            ...
                                            if (
                                              filters.assignedUser &&
                                              !['all-lawyers', 'all-ops'].includes(filters.assignedUser)
                                            ) {
                              ...
                              <div className="text-center">
                                ...
                                <div className="d-flex justify-content-around mb-3">
                                  <div className="text-center">
                                    <div className="text-white fw-bold fs-3">
                                      {casesData.backendClosedPositive || 0}
                                    </div>
                                    <div className="text-white-50 small">Won</div>
                                  </div>
                                  <div className="text-center">
                                    <div className="text-white fw-bold fs-3">
                                      {casesData.backendClosedNegative || 0}
                                    </div>
                                    <div className="text-white-50 small">Lost</div>
                                  </div>
                                  <div className="text-center">
                                    <div className="text-white fw-bold fs-3">
                                      {(casesData.backendClosedPositive || 0) + (casesData.backendClosedNegative || 0)}
                                    </div>
                                ...
                                <div className="border-top border-white border-opacity-25 pt-2">
                                  <p className="text-white mb-0 small">
                                    {casesData.backendClosedPositive || 0} won /{' '}
                                    {(casesData.backendClosedPositive || 0) + (casesData.backendClosedNegative || 0)}{' '}
                                    closed cases
                                  </p>
                                </div>
