...
const ReceptionistCalendar = ({ token }) => {
   ...
   const handleConfirm = async () => {
    ...
    const handleSelect = async (selectedOption) => {
        ...
        if (!isCalender) {
            ...
            if (selectedOption?.UserName === "All") {
                ...
                axios.get(`${ApiEndPoint}GetAllFreeAppointments/${formattedDate}`)
                    .then((res) => {
                ...
                const formattedDate = selectedDate.toISOString().split("T")[0]
                axios.get(`${ApiEndPoint}GetAllFreeAppointments/${formattedDate}?lawyerId=${selectedOption?._id}`)
                    .then((res) => {
                        console.log(res.data)
                        setAllSlots(res.data)
                    })
    ...
    const handleGetFreeSlots = (date) => {
        ...
        axios.get(`${ApiEndPoint}${apiaddress}`)
            .then((res) => {
                console.log(res.data)
                setAllSlots(res.data)
            })
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const responseupdate = await fetch(
                    `${ApiEndPoint}Bookappointments/${lawyerId}/${slot?._doc?._id}`,
                    {
                        method: "PATCH",
                        headers: {
                            "Content-Type": "application/json",
    ...
    return (
        ...
        >
            <Row className="m-0 p-1 gap-3 gap-md-5 justify-content-center" style={{ height: "84vh" }}>
                <Col
                    xs={12}
                    md={10}
                    lg={10}
