...
const ContactForm = ({ users, participants }) => {
  ...
  const validateFileSize = (file) => {
    ...
    return fileType !== 'other' && file.size <= (sizeLimits[fileType] || 2 * 1024 * 1024);
  };
  ...
  const handleFileChange = (input) => {
    ...
    for (let file of files) {
      ...
      if (file.name.length > 40) {
        ...
        let truncatedName = file.name.substring(0, 20) + '...' + file.name.slice(-10);
      ...
      if (fileType === 'other') {
        ...
        invalidSizeFiles.push(
          `${file.name} (Max ${((sizeLimits[fileType] || 2 * 1024 * 1024) / (1024 * 1024)).toFixed(1)}MB)` // Show max limit
        );
      } else {
  ...
  const fetchClientDetails = async () => {
    ...
    try {
      ...
      await setClientDetails(
        response.data?.clientDetails ? response.data?.clientDetails : response.data?.lawyerDetails
    ...
    let apifuncation = role === 'lawyer' ? 'geLawyerDetails' : 'getClientDetails';

    try {
      const response = await axios.get(`${ApiEndPoint}getUserById/${clientDetails?._id}`);
  ...
  const ContactRow = ({ label, value, isEmail, isSmall }) => (
    <tr className="align-middle" style={{ height: '3rem' }}>
      ...
      >
        {isEmail ? (
          value ? (
            <a href={`mailto:${value}`} className="text-white text-decoration-none">
              {value}
            </a>
          ) : (
            'N/A'
        ...
        >
          ...
          <div className="mb-2 mt-2 text-center avatar-container">
            <label htmlFor="profilePicInput">
              <img
                src={usersDetails?.ProfilePicture ? usersDetails?.ProfilePicture : defaultProfilePic}
                alt="Profile"
                style={{
                  maxWidth: '80px',
                  maxHeight: '80px',
            ...
            <table className="w-100" style={{ borderCollapse: 'collapse' }}>
              <tbody>
                ...
                {usersDetails?.Role !== 'client' && (
                  <>
                    <InfoRow label="Position" value={clientDetails?.Position} icon={faBriefcase} />
                    <InfoRow label="Bio" value={clientDetails?.Bio} icon={faUser} />
                    <InfoRow label="Department" value={clientDetails?.Department} icon={faBuilding} />
                    <InfoRow label="Expertise" value={clientDetails?.Expertise} icon={faLightbulb} />
                  </>
                )}

                {/* Contact Details */}
                <ContactRow icon={faEnvelope} label="Email" value={usersDetails?.Email} isEmail />
                <ContactRow icon={faPhone} label="Contact" value={clientDetails?.Contact} />
                <ContactRow icon={faAddressCard} label="Address" value={clientDetails?.Address} isSmall />
              </tbody>
            </table>
          </div>
        </Col>
