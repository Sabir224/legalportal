...
import {
    ...
    faSpinner,
    faCalendar,
    faEnvelope, faBriefcase, faUser, faBuilding, faLightbulb
    ...
    const fetchClientDetails = async () => {
        ...
        try {
            const response = await axios.get(
                `${ApiEndPoint}getUserById/${users}`
            );
            await setUsersDetails(response.data);
            role = response.data?.Role
            console.log(" response.data ", response.data)
            await setClientDetails(response.data?.clientDetails ? response.data?.clientDetails : response.data?.lawyerDetails); // Set the API response to state
    ...
    const ContactRow = ({ icon, label, value, isEmail, isSmall }) => (
        <tr>
            ...
            <td className="p-2 text-white">
                {isEmail ? (
                    <a href={`mailto:${value}`} style={{ color: "white", textDecoration: "none" }}>{value}</a>
                ) : (
