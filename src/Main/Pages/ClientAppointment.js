...
const ClientAppointment = () => {
  ...
  const fetchLawyerDetails = async () => {
    ...
    try {
      ...
      const lawyerIdRes = await axios.get(`${ApiEndPoint}getCaseClientAndLawyerIds/${caseIdToUse}`);
      ...
      const lawyerDetailsRes = await axios.get(`${ApiEndPoint}getLawyerDetailsById/${lawyerid}`);
      ...
      const clientDetailsRes = await axios.get(`${ApiEndPoint}getClientDetailsByUserId/${effectiveCaseInfo?.ClientId}`);
          ...
          <div className="profile-section p-3 h-100 d-flex flex-column mt-4">
            ...
            <div className="lawyer-details mt-3 flex-grow-1">
              ...
              <div className="d-flex align-items-center mb-2 flex-wrap">
                ...
                <p className="m-0" style={{ wordBreak: 'break-word' }}>
                  <a
                    href={`http://maps.google.com/?q=${encodeURIComponent(lawyerDetails?.Address)}`}
                    target="_blank"
        ...
        >
          ...
          <div className="d-flex flex-wrap">
            {calendarDates.map((date, index) => {
              if (!date) {
                return (
                  <div key={index} className="calendarEmpty" style={{ width: 'calc(100% / 7)', height: '40px' }}></div>
                );
              }
              ...
              return (
                <div
                  ...
                  style={{
                    border:
                      selectedDate?.getDate() === date?.getDate() ? '2px solid white' : '2px solid rgb(2, 30, 58)',
                    borderRadius: '5px',
