...
const LEA_Form = ({ token }) => {
  ...
  // UPDATED: Function to update section with services - FIXED: Combined VAT display for legal services
  const updateSectionWithServices = (section, servicesData) => {
    ...
    const servicePoints = servicesData.map((service) => {
      ...
      // For legal services, show service with AED amount and VAT combined
      if (service.serviceCategory !== 'corporate') {
        const serviceFee = parseInt(service.serviceFees) || 0;
        let vatText = '';
        ...
        if (service.vatCharges && service.vatCharges !== '') {
          ...
          if (service.vatCharges.includes('%')) {
            const vatPercentage = parseInt(service.vatCharges) || 0;
            vatAmount = Math.round((serviceFee * vatPercentage) / 100);
            vatText = ` + ${service.vatCharges} VAT (AED ${vatAmount})`;
          } else {
            vatAmount = parseInt(service.vatCharges) || 0;
            vatText = ` + VAT (AED ${vatAmount})`;
          }
        }

        serviceText = `<strong>${service.selectedService}: AED ${service.serviceFees}${vatText}</strong><br/>`;
      } else {
        // For corporate services, show service name only
              ...
              <div key={service.id} className="border rounded p-3 mb-3">
                ...
                {service.serviceCategory === 'corporate' && (
                  <>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Text className="text-muted">Authority costs are VAT exempt.</Form.Text>
                    </Form.Group>
                ...
                <div className="border-top pt-2 mb-3">
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <span className="text-success fw-bold">AED {calculateServiceTotal(service)}</span>
                  </div>
                </div>
