...
const LEA_Form = ({ token }) => {
  ...
  const updateSectionWithServices = (section, servicesData) => {
    ...
    const servicePoints = servicesData.map((service) => {
      ...
      // NEW: Build the service text with VAT and AWS fees
      let serviceText = `${service.selectedService}: <strong>AED ${service.serviceFees}</strong>`;

      // Add VAT charges if provided (now supports percentages)
      if (service.vatCharges && service.vatCharges !== '') {
        serviceText += ` (VAT Charges: ${service.vatCharges})`;
      }

      // Add AWS fees if provided
      if (service.awsFees && service.awsFees !== '') {
        serviceText += ` (AWS Fees: ${service.awsFees})`;
      }
    ...
    const existingNonServicePoints = currentSection.points.filter((point) => {
      ...
      const isServicePoint =
        point.text &&
        (point.text.includes(': <strong>AED ') || /^[^:]+:\s*<strong>AED\s*[\d,]+\.?\d*<\/strong>/.test(point.text));
    ...
    section.points.forEach((point) => {
      if (point.text && point.text.trim() !== '') {
        ...
        if (serviceMatch) {
          ...
          // NEW: Parse VAT and AWS fees from additional text if they exist
          let vatCharges = '';
          let awsFees = '';

          if (additionalText) {
            // Updated: Parse VAT charges that could be percentage or fixed amount
            const vatMatch = additionalText.match(/\(VAT Charges:\s*([^)]+)\)/);
            const awsMatch = additionalText.match(/\(AWS Fees:\s*([^)]+)\)/);

            if (vatMatch) vatCharges = vatMatch[1].trim();
            if (awsMatch) awsFees = awsMatch[1].trim();
          }
          ...
          services.push({
            ...
            vatCharges: vatCharges, // NEW: VAT Charges field (now supports percentages)
            awsFees: awsFees, // NEW: AWS Fees field
  ...
  const handleDownload = async () => {
    try {
      ...
      const footerContent =
        serviceType === 'aws'
          ? 'P/O Box 96070\nDubai: 1 Sheikh Zayed Road, The H Dubai, Floor 1602\nTel: (04) 5547635, +971 527652221\nemail: info@awsadvocates.co'
      ...
      const docDefinition = {
        ...
        content: [
          ...
          ...fixedHeadings.flatMap((section, sectionIndex) => [
            ...
            {
              ol: section.points.map((point) => {
                if (!point.text) return { text: '' };

                if (point.text.includes('<strong>')) {
                  // Extract text before and after bold section
      ...
      <Modal show={showRejectModal} onHide={() => setShowRejectModal(false)}>
        ...
        <Modal.Footer className="justify-content-center">
          <div className="d-flex gap-3">
            ...
            <Button variant="danger" onClick={handleRejectLFA} disabled={isSubmittingRejection}>
