...
export default function TaskList({ token }) {
  ...
  return (
    ...
    onChange={(e) => {
      ...
      if (newValue && newValue !== assignedUserId) {
        setAssignedUserId(newValue);
        handleFieldBlur(todo._id?.value || todo.id, 'assignedUsers', newValue, false, null);
      }
      setEditingAssignedUser(null); // ✅ close dropdown
    }}
    onClose={() => {
      setEditingAssignedUser(null); // ✅ value stays if not selected
    }}
    displayEmpty
    renderValue={(selected) => {
      if (!selected) {
        return <em style={{ color: 'rgba(0,0,0,0.4)' }}>Assign User</em>;
      }
      const user = users.find((u) => u.id === selected);
      return user ? `${user.UserName} (${capitalizeFirst(user.Role)})` : '';
    }}
    IconComponent={(props) => (
      <ExpandMore {...props} sx={{ color: '#D4AF37' }} />
    )}
    sx={{
      '& .MuiSelect-select': {
        py: 1,
      ...
      PaperProps: {
        sx: {
          ...
          '& .MuiMenuItem-root': {
            ...
            '&.Mui-selected': { backgroundColor: 'rgba(212, 175, 55, 0.2)' },
          },
        },
      },
    }}
